...
export async function GET() {
  ...
  try {
    ...
    // ========== QUOTATION SYSTEM TESTS ==========
    console.log("Testing Quotation System...");
    
    try {
      // Test quotation_requests table
      const { data: quotationTableCheck, error: quotationTableError } = await supabase
        .from('quotation_requests')
        .select('count(*)')
        .limit(1);
      
      if (quotationTableError) {
        testResults.quotationSystem.tableCheck = `❌ Error: ${quotationTableError.message}`;
        testResults.overallSuccess = false;
      } else {
        testResults.quotationSystem.tableCheck = "✅ quotation_requests table exists";
        
        // Test quotation insert
        const quotationTestData = {
          client_name: "Test Quotation User",
          client_email: "test-quotation@example.com",
          project_title: "Test Quotation Project",
          print_type: "digital",
          paper_type: "standard", 
          paper_size: "A4",
          quantity: 100,
        ...
        const { data: quotationInsertData, error: quotationInsertError } = await supabase
          .from('quotation_requests')
          .insert([quotationTestData])
          .select()
          .single();

        if (quotationInsertError) {
          testResults.quotationSystem.insertTest = `❌ Insert failed: ${quotationInsertError.message}`;
          testResults.overallSuccess = false;
        } else {
          testResults.quotationSystem.insertTest = "✅ Can insert quotation records";
          
          // Test quotation read
          const { data: quotationReadData, error: quotationReadError } = await supabase
            .from('quotation_requests')
           .select('*')
           .eq('id', quotationInsertData.id)
           .single();

         if (quotationReadError) {
           testResults.quotationSystem.readTest = `❌ Read failed: ${quotationReadError.message}`;
           testResults.overallSuccess = false;
         } else {
           testResults.quotationSystem.readTest = "✅ Can read quotation records";
         }

         // Clean up quotation test record
         ...
         if (quotationDeleteError) {
           testResults.quotationSystem.cleanup = "⚠️ Cleanup failed";
         } else {
           testResults.quotationSystem.cleanup = "✅ Test record cleaned up";
         }
       }
     }
   } catch (quotationError: any) {
     testResults.quotationSystem.error = `❌ Exception: ${quotationError.message}`;
     testResults.overallSuccess = false;
   }
   ...
   try {
     ...
     const { data: jobSheetTableCheck, error: jobSheetTableError } = await supabase
       .from('job_sheets')
       .select('count(*)')
       .limit(1);
     ...
     if (jobSheetTableError) {
       ...
       const { data: jobSheetInsertData, error: jobSheetInsertError } = await supabase
         .from('job_sheets')
       ...
       if (jobSheetInsertError) {
         ...
         testResults.jobSheetSystem.insertTest = "✅ Can insert job sheet records";
         
         // Test job sheet read
         const { data: jobSheetReadData, error: jobSheetReadError } = await supabase
           .from('job_sheets')
           .select('*')
         ...
         if (jobSheetReadError) {
           ...
           testResults.jobSheetSystem.readTest = "✅ Can read job sheet records";
         }
         ...
         if (jobSheetUpdateError) {
           ...
           testResults.jobSheetSystem.updateTest = "✅ Can update job sheet records";
         }
   ...
   // Test quotation_notes table
   try {
     const { error: quotationNotesError } = await supabase
       .from('quotation_notes')
       .select('count(*)')
       .limit(1);
     
     if (quotationNotesError) {
       testResults.optionalTables.quotationNotes = "⚠️ quotation_notes table not found or not accessible";
     } else {
       testResults.optionalTables.quotationNotes = "✅ quotation_notes table exists";
     }
   } catch {
     testResults.optionalTables.quotationNotes = "⚠️ quotation_notes table not found";
   }
   ...
   try {
     ...
     if (jobSheetNotesError) {
       testResults.optionalTables.jobSheetNotes = "⚠️ job_sheet_notes table not found or not accessible";
     } else {
       testResults.optionalTables.jobSheetNotes = "✅ job_sheet_notes table exists";
     }
   } catch {
     testResults.optionalTables.jobSheetNotes = "⚠️ job_sheet_notes table not found";
   }
   ...
   try {
     ...
     if (invoicesError) {
       testResults.optionalTables.invoices = "⚠️ invoices table not found or not accessible";
     } else {
